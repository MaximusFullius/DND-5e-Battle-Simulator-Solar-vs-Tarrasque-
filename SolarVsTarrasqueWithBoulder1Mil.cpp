    ...
    while (tarrasqueHealth > 0 && solarHealth > 0){
        ...
        if (solarHealth > 0){
            if (solarHealth <= 75 && solarHealCharges > 0){
                ...
                if (tarrasqueHealth > 0){ // Searing Burst
                    //cout << "The solar flies close to the Tarrasque and casts Searing Burst before teleporting away!!\n";
                    // Implementation of Fly close + Searing Burst + Teleport. Searing Burst does 4d6 + 4d6 on a failed DC 23 dex save or half on success.
                    for (int i = 1; i <=4; i++){
                        solarBonusAttackDamage += d6(gen);
                    }
                    // Tarrasque has advantage on saving throws against magic.
                    conSave1 = d20(gen);
                    conSave2 = d20(gen);
                    if (conSave1 >= conSave2){
                        conSave = conSave1;
                    }
                    else{
                        conSave = conSave2;
                    ...
                    if (conSave == 20){
                        //cout << "Tarrasque SUCCEEDS the Dex save against the Solars' Searing Burst attack! The Searing Burst deals only half damage to the Tarrasque!\n";
                        solarBonusAttackDamage = (solarBonusAttackDamage / 2.0);
                        //cout << "The Searing Burst attack deals " << (int)solarBonusAttackDamage << " damage to the Terrasque for a total of " << ((int)solarBonusAttackDamage + solarAttackDamage) << " damage! ";
                        tarrasqueHealth -= (solarAttackDamage + (int)solarBonusAttackDamage);
                        //cout << "The Tarrasque has " << tarrasqueHealth << " hp remaining.\n";
                    }
                    else{
                        //cout << "Tarrasque FAILS the Dex save against the Solars' Searing Burst attack! \n";
                        if (tarrasqueHealth - (solarAttackDamage + (int)solarBonusAttackDamage) <= 20){ // If the Tarrasque fails the dex save, and the attack would kill it or, 
                                                                                                // leave it with low health, then try using a legendary resistance charge.
                            if (tarrasqueLegendaryResistancesRemaining > 0){
                                //cout << "The Tarrasque uses a Legendary Resistance charge to halve the damage from the Searing Burst!\n";
                                tarrasqueLegendaryResistancesRemaining--;
                                solarBonusAttackDamage = (solarBonusAttackDamage / 2.0);
                            }
                        }
...
int main(){
    ...
    int solarWins = 0;
    int tarrasqueWins = 0;
    int numBattles = 1000000;
    int interval = 100000;
    for (int n = 0; n < numBattles; n++){
        result = fight();
        if (result == 1){
            solarWins++;
        }
        ...
        if (n % interval == 0) cout << n << "\n";
    }
    double solarPercentage = (solarWins / 1000000.0) * 100.0;
    double tarrasquePercentage = (tarrasqueWins / 1000000.0) * 100.0;
    cout << "Solar wins: " << solarWins << "(" << solarPercentage << " percent) | Tarasque wins: " << tarrasqueWins << "(" << tarrasquePercentage  << " percent) \n";
    return 0;
}
